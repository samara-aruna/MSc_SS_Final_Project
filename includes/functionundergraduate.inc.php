...
{
    $sql = "SELECT * FROM undergraduate WHERE usersUid = ? OR usersEmail = ?;";
...
{
    $sql = "INSERT INTO undergraduate (usersFname, usersLname, usersUid, usersEmail,	userPwd) VALUES (?,?,?,?,?);";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql))
    {
        header("Location:../undergraduate.php?error=stmtfaild");
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location:../loginundergraduate.php?error=none");
    exit(); 
}
...
function loginUser($conn,$username,$pwd) 
{
    ...
    if($uidExists === false)
    {
        header("Location:../undergraduate.php?error=wronglogin");
        exit();
    }
    ...
    if($checkPwd === false)
    {
        header("Location:../loginundergraduate.php?error=wrongloging");
        exit();
    }
    else if($checkPwd === true)
    {
        ...
        header("Location:../indexuniversity.php");
        exit();
    }
}
