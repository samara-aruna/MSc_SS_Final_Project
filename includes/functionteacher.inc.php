...
{
    $sql = "SELECT * FROM teacher WHERE usersUid = ? OR usersEmail = ?;";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql))
    {
        header("Location:../teacher.php?error=stmtfaild");
        exit();
    }
...
{
    $sql = "INSERT INTO teacher (usersFname, usersLname, usersUid, usersEmail,	userPwd) VALUES (?,?,?,?,?);";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql))
    {
        header("Location:../teacher.php?error=stmtfaild");
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location:../loginteacher.php?error=none");
    exit(); 
}
...
function loginUser($conn,$username,$pwd) 
{
    ...
    if($uidExists === false)
    {
        header("Location:../teacher.php?error=wronglogin");
        exit();
    }
    ...
    if($checkPwd === false)
    {
        header("Location:../loginteacher.php?error=wrongloging");
        exit();
    }
