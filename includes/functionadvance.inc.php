...
{
    $sql = "SELECT * FROM advance WHERE usersUid = ? OR usersEmail = ?;";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql))
    {
        header("Location:../advance.php?error=stmtfaild");
        exit();
    }
...
{
    $sql = "INSERT INTO advance (usersFname, usersLname, usersUid, usersEmail,	userPwd) VALUES (?,?,?,?,?);";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql))
    {
        header("Location:../advance.php?error=stmtfaild");
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location:../loginadvance.php?error=none");
    exit(); 
}
...
function loginUser($conn,$username,$pwd) 
{
    ...
    if($uidExists === false)
    {
        header("Location:../advance.php?error=wronglogin");
        exit();
    }
    ...
    if($checkPwd === false)
    {
        header("Location:../loginadvance.php?error=wrongloging");
        exit();
    }
    else if($checkPwd === true)
    {
        ...
        header("Location:../indexAlevel.php");
        exit();
    }
}
