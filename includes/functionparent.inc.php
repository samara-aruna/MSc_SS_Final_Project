...
{
    $sql = "SELECT * FROM parent WHERE usersUid = ? OR usersEmail = ?;";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql))
    {
        header("Location:../parent.php?error=stmtfaild");
        exit();
    }
...
{
    $sql = "INSERT INTO parent (usersFname, usersLname, usersUid, usersEmail,	userPwd) VALUES (?,?,?,?,?);";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql))
    {
        header("Location:../parent.php?error=stmtfaild");
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location:../loginparent.php?error=none");
    exit(); 
}
...
function loginUser($conn,$username,$pwd) 
{
    ...
    if($uidExists === false)
    {
        header("Location:../parent.php?error=wronglogin");
        exit();
    }
    ...
    if($checkPwd === false)
    {
        header("Location:../loginparent.php?error=wrongloging");
        exit();
    }
